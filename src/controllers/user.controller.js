...
if(req.file && Array.isArray(req.files.coverImage) && req.files.coverImage.length > 0){
    ...
    if(!createdUser){
        throw new ApiError(500,"something went wrong when register ")
    }
...
try {
        ...
        .status(200
            ...
            .json(
                new ApiResponse(
                    200,
                    {accessToken , refreshToken:newRefreshToken},"Access token refresh token"
                )
            )
        )
} catch (error) {
...
const updateAccountDetails = asyncHandler(async(req,res)=>{
    ...
    return res
    .status(200)
    .json(new ApiResponse(2000,user,"Account details updated successfully"))
})
